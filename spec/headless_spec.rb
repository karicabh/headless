...
describe Headless do
  ...
  describe 'launch options' do
    ...
    it "starts Xvfb" do
      expect(Process).to receive(:spawn).with(*(%w(/usr/bin/Xvfb :99 -screen 0 1280x1024x24)+[hash_including(:err)])).and_return(123)
      headless = Headless.new
    end

    it "allows setting screen dimensions" do
      expect(Process).to receive(:spawn).with(*(%w(/usr/bin/Xvfb :99 -screen 0 1024x768x16)+[hash_including(:err)])).and_return(123)
      headless = Headless.new(:dimensions => "1024x768x16")
    end

    it "allows to enable extensions", focus: true do
      expect(Process).to receive(:spawn).with(*(%w(/usr/bin/Xvfb :99 -screen 0 1280x1024x24 +iglx)+[hash_including(:err)])).and_return(123)
      headless = Headless.new(:extensions => [:iglx])

      expect(Process).to receive(:spawn).with(*(%w(/usr/bin/Xvfb :99 -screen 0 1280x1024x24 +iglx)+[hash_including(:err)])).and_return(123)
      headless = Headless.new(:extensions => 'iglx')

      expect(Process).to receive(:spawn).with(*(%w(/usr/bin/Xvfb :99 -screen 0 1280x1024x24 +iglx +dummy)+[hash_including(:err)])).and_return(123)
